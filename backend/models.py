# 기존에 잘 돌아가는 코드일까봐 혹시 몰라서 주석처리 models.py
...
class Question(Base):
    __tablename__ = "questions"
    question_id = Column(Integer, primary_key=True, index=True)
    number = Column(Integer, nullable=False)  # 번호(개념/슬라이드/챕터 등)
    slide_id = Column(Integer, ForeignKey("slides.slide_id"), nullable=False)
    question_type = Column(Enum('객관식', '주관식', '참거짓', '빈칸채우기'), nullable=False)
    content = Column(Text, nullable=False)
    answer = Column(Text, nullable=False)
    explanation = Column(Text)
    created_at = Column(TIMESTAMP, server_default=text("CURRENT_TIMESTAMP"))
    difficulty = Column(String(255), nullable=False)  # 난이도(상/중/하)
...
class Keyword(Base):
    __tablename__ = "keywords"
    keyword_id = Column(BigInteger, primary_key=True, autoincrement=True)
